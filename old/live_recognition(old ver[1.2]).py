...
# ================== 初始化 ONNX 推論 (自動選擇 GPU/CPU) ==================
def create_ort_session(model_path):
    """嘗試使用 OpenVINO，失敗則退回 CPU（隱藏錯誤訊息）"""
    import sys, io
    old_stderr = sys.stderr
    sys.stderr = io.StringIO()  # 抑制 OpenVINO 的 DLL 缺失錯誤
    try:
        session = ort.InferenceSession(model_path, providers=['OpenVINOExecutionProvider', 'CPUExecutionProvider'])
        if 'OpenVINO' in str(session.get_providers()):
            print("🚀 成功啟用 OpenVINO GPU 加速")
        else:
            print("💻 OpenVINO 不可用，使用 CPU 模式")
    except Exception:
        print("💻 OpenVINO 無法載入，使用 CPU 模式")
